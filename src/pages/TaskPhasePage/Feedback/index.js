...
import {
    getLastSeenMessageByProject, getNewerMessageByProject,
    getOlderMessageByProject,
    getPinMessageByProject,
    getSourceMessage
} from "../../../api/messageApi";
...
import SearchMessageList from "../../ChatboxPage/SearchMessageList";
import MediaMessageList from "../../ChatboxPage/MediaMessageList";
import MessagePinSection from "../../ChatboxPage/MessagePinSection";
import PinMessageList from "../../ChatboxPage/PinMessageList";
import BarProgress from "../../../components/BarProgress";
import MessageItem from "../../ChatboxPage/MessageItem";
import TypingIndicator from "../../ChatboxPage/TypingIndicator";
import {Button} from "primereact/button";
import TextareaAutosize from "react-textarea-autosize";
import MediaCard from "../../../components/MediaCard";
import {Card} from "primereact/card";

const Feedback = ({onClose, taskData}) => {
    const {connected, connect, disconnect, subscribe, unsubscribe, sendMessage, seenMessage, typingMessage, pinMessage, storeMediaMessage} = useWebSocketChat();
    ...
    useEffect(() => {
        if (connected && taskData?.id && isReady) {
            const topic = `/public/task/${taskData.id}`;
            const seenMessageTopic = `/public/task/${taskData.id}/read`;
            const typingMessageTopic = `/public/task/${taskData.id}/typing`;
            const pinMessageTopic  = `/public/task/${taskData.id}/pin`;
            const storageMessageTopic =  `/public/task/${taskData.id}/storage`;
    ...
    const loadLastSeenMessages = async (userMap) => {
        try {
            ...
            const response = await getLastSeenMessageByProject(usernameList, project?.id, taskData?.id);
    ...
    const fetchOlderMessages = async (projectId, lastMsgId) => {
        try {
            setLoading(true);
            const response = await getOlderMessageByProject(projectId, lastMsgId, 0, 0, "sentTime,DESC", taskData?.id);
    ...
    const handleFileSelect = (event, mode) => {
        ...
        if (!allowedExtensions[mode].includes(ext)) {
            alert(t('chatboxPage.errors.unsupportedFiles'));
            event.target.value = null;
            return;
        }

        handleSendSingleFile(file);
        event.target.value = null;
    };

    const handleSendSingleFile = async (file) => {
        ...
        setMessages(prevMessages => [fakeMessage, ...prevMessages]);

        try {
            await handleSendMediaMessage(file, fakeId);
        } catch (error) {
            console.error("Error sending file:", error);
            // Remove fake message on error
            setMessages(prevMessages => prevMessages.filter(msg => msg.fakeId !== fakeId));
        }
    };
            ...
            <div className="feedback-header">
                <div className="feedback-header-content">
                    <div className="feedback-header-title">
                        {taskData.name.toUpperCase()}
                    </div>
                    <Button
                        icon="pi pi-times"
                        rounded
                        outlined
                        className="feedback-header-close-button"
                        onClick={onClose}
                        aria-label="Close"
                    />
                </div>
