...
const DashboardTaskTimeAllocation = ({ taskList, phaseList }) => {
    ...
    return (
        <div className="dashboard-task-time-container">
            ...
            <div className="dashboard-task-time-content" style={{ height: '300px', width: '25rem', overflowX: 'auto', overflowY: 'hidden' }}>
                {chartData.length > 0 ? (
                    <div style={{ width: `${Math.max(chartData.length * 80, 600)}px`, height: '100%' }}>
                        <ResponsiveBar
                            data={chartData}
                            keys={['hours']}
                            indexBy="id"
